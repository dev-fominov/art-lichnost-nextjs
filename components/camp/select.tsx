...
export const Select = ({data, setFieldValue, name, title}: any) => {
    ...
    return <div className={styles.filterSelect}>
        <div className={styles.itemFilter}
             ref={wrapperRef}
             onClick={() => setOpenSelect(!openSelect)}>{name}
            {openSelect
                ? <IoIosArrowRoundUp className={styles.arrow}/>
                : <IoIosArrowRoundDown className={styles.arrow}/>}</div>
        <ul  className={openSelect ? styles.ulSortActive : styles.ulSort}>
            ...
            }} key={el.id}><label className={styles.radioLabel}>
                {name === el.name
